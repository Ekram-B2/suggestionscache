...
func decodeGzip(reader io.Reader) (rankManager.Rank, error) {
	...
	if err != nil {
		l4g.Error("OPERATION-ERROR: unable to decompress representation backing reader drawn from cache: %s", err.Error())
	...
	if err != nil {
		l4g.Error("OPERATION-ERROR: unable to decode reader to go structure: %s", err.Error())
		return rankManager.Rank{}, err
	}
...
func decodeRaw(reader io.Reader) (rankManager.Rank, error) {
	...
	if err != nil {
		l4g.Error("OPERATION-ERROR: unable to decode data backing reader %s", err.Error())
		return rankManager.Rank{}, err
	}
	// 2. Return failure case if the software was unable to pipe data into byte slice
	if err != nil {
		l4g.Error("OPERATION-ERROR: unable to pipe representation backing reader into a byte slice: %s", err.Error())
		return rankManager.Rank{}, err
	}
	return rank, nil
}
