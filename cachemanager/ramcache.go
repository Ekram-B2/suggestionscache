...
func (rm ramCacheManager) getRankFromCache(key, byteDecoderType string) (isCacheHit bool, returnedRank rankManager.Rank, err error) {
	...
	returnedRank, err = getByteDecoder(os.Getenv("OPERATION_TYPE"))(reader)
	if err != nil {
		l4g.Error("unable to get rank from the decoder: %s", err.Error())
...
func (rm ramCacheManager) putInCache(key string, value rankManager.Rank, byteEncoder byteEncoder) error {
	...
	valueStream, err := getByteEncoder(os.Getenv("OPERATION_TYPE"), value)(value)
	if err != nil {
		l4g.Error("unable to get byte stream from the encoder: %s", err.Error())
		return err
	}
