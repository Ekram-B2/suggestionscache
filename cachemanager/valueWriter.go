...
func encodeGzip(rank rankManager.Rank) byteEncoder {
	return func(rankManager.Rank) (compressedBytes []byte, err error) {
		...
		if err != nil {
			l4g.Error("unable to get uncompressed byte stream from the rank")
			return nil, err
		}
		...
		if err != nil {
			l4g.Error("unable write compressed bytes to backing byte slice")
			return nil, err
		}
		writer.Close()
...
func encodeRaw(rank rankManager.Rank) ([]byte, error) {
	...
	if err != nil {
		l4g.Error("unable to marshall rank into a byte stream")
		return nil, err
	}
